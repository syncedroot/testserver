...
life_action_gather = true; //Lock out the interaction menu for a bit..
_weight = [_vehicle] call life_fnc_vehicleWeight;
if((_weight select 1) >= (_weight select 0)) exitWith {hint localize "STR_NOTF_DeviceFull"; life_action_gather = false;};
...
if(_item == "") exitWith {hint "Bad Resource?"; life_action_gather = false;};
...
life_action_gather = false; //Unlock it since it's going to do it's own thing...
